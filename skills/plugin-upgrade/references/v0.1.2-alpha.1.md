...
- **实战批注**（2026-08-30，容器全链路验证）: 迁移前先判定插件运行在哪个平面——
  ...
  完整过程见仓库 [docs/validation-report-2026-08-30.md](https://github.com/oh-my-dsh/dsh-plugin-upgrade-skill/blob/main/docs/validation-report-2026-08-30.md)。
...
- **迁移配方**: 要保持 rc.2 的关闭立场，在自己的 `cordis.patch.yml` 覆盖：`session-telemetry-otel` 必须重述**整个** `config` 对象（include patch 是替换不是合并），`mode` 写 `process.env.DSH_TELEMETRY_MODE || 'DISABLED'`；`plugin-package-inventory-deepseek` 行 `config: { enabled: false }`。开不开由部署方按隐私政策定，本卡只保证默认值变化被看见。
